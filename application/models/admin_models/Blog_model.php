...
class Blog_model extends CC_Model { 
    public function __construct() {
        parent::__construct();
    } 
    private $_blog = 'tbl_blog'; 
	...
	public function store_blog($data) { 
        $this->db->insert($this->_blog, $data); 
    ...
    public function get_blog_info($page='1',$search="",$status='') {
		if($page<1){
			 $page=1;
		 } 
	...
	public function get_blog_count($search="",$status='') {
		         $this->db->select('*') 
                ->from('tbl_blog')  
                ->where('deletion_status', 0) 
				->order_by('id', 'asc');
    ...
    public function get_blog_by_id($blog_id) {
        $result = $this->db->get_where($this->_blog, array('id' => $blog_id, 'deletion_status' => 0));
    ...
    public function update_blog($blog_id, $data) {
        $this->db->update($this->_blog, $data, array('id' => $blog_id));
        return $this->db->affected_rows();
    }

    public function remove_blog_by_id($blog_id) {
        $this->db->update($this->_blog, array('deletion_status' => 1), array('id' => $blog_id));
        return $this->db->affected_rows();
    }
